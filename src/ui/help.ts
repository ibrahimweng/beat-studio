...
const SECTIONS: readonly Section[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      ['Find sounds online', 'Searches Freesound from inside the app. Needs a free API key from freesound.org/apiv2/apply/, which is kept in this browser and sent nowhere else.'],
      ['Which licence to search', 'It defaults to CC0, which asks nothing of you. CC-BY is free to use anywhere as long as you name the author, and the app records who that is. Widen it only if you know what the result obliges you to do.'],
      ['They are previews', 'Freesound hands out a 128 kbps preview without signing in; the master needs an account and a download. A preview is enough to place against picture and is not enough to master from, so each one is tagged “preview”. Download the original from the site and drop it in when you need it.'],
      ['If the search will not connect', 'The app cannot tell an offline browser from a request the browser blocked before it left, so it says both. Everything downloaded by hand still works.'],
